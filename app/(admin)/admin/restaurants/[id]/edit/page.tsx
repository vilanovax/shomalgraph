...
export default function EditRestaurantPage() {
    ...
    async function fetchRestaurant() {
      try {
        ...
        if (!response.ok || !data.success) {
          throw new Error(data.error || "خطا در دریافت اطلاعات رستوران");
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "خطا در ویرایش رستوران");
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border bg-gradient-to-l from-blue-600 via-indigo-600 to-purple-500 px-6 py-8 text-white shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm text-white/70">پنل مدیریت · ویرایش</p>
            <h1 className="mt-1 text-3xl font-black">ویرایش رستوران</h1>
            <p className="text-white/80">
              اطلاعات رستوران را به‌روزرسانی کنید و تغییرات را ذخیره کنید.
            </p>
          </div>
          <Button
            onClick={() => router.back()}
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 gap-2 bg-gradient-to-l from-blue-600 to-indigo-500 text-white shadow-lg hover:opacity-90"
                >
