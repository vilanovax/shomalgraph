...
export default function NewListPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // تولید slug از title اگر خالی باشد
      ...
      router.push(`/admin/lists/${data.data.id}`);
      router.refresh();
    } catch (error: unknown) {
      setError(
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border bg-gradient-to-l from-purple-600 via-pink-600 to-rose-500 px-6 py-8 text-white shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm text-white/70">پنل مدیریت · ایجاد لیست</p>
            <h1 className="mt-1 text-3xl font-black">ایجاد لیست جدید</h1>
            <p className="text-white/80">
              لیست جدیدی برای اکسپلور ایجاد کنید و آیتم‌ها را اضافه کنید.
            </p>
          </div>
          <Button
            onClick={() => router.back()}
        ...
        <div className="grid gap-3 text-sm text-white/80 sm:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            ...
            <p className="mt-2 text-lg font-semibold">
              {isFormReady ? "آماده ایجاد" : "نیاز به عنوان"}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            <p className="text-xs uppercase tracking-widest text-white/70">
      ...
      <Card className="border-0 shadow-xl">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="slug">Slug (اختیاری)</Label>
              <Input
                id="slug"
                value={formData.slug}
              ...
              <p className="text-xs text-muted-foreground">
                اگر خالی بگذارید، از عنوان به صورت خودکار تولید می‌شود
              </p>
            </div>
